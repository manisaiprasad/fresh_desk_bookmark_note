...
function insertRow(freshdeskDomainName, ticketId, ticketSubject) {
    ...
    ticketSubject = ticketSubject.split("$$") 

    c1.innerHTML ='<div class="card"><div class="card-body"><h4 class="card-title">'+ticketSubject[0]+'</h4><h5 class="card-subtitle mb-2 text-muted">[Ticket:'+ticketId+']<a target="blank" href="https://' + freshdeskDomainName + '/a/tickets/'
    + ticketId + '">' +ticketSubject[1] + '</a></br> </h5> <button type="button" id="' + ticketId + '" onclick="removeTicket(this);" class="btn btn-danger">Delete Bookmark</button></div></div>';
}
    ...
    getLoggedInUser().then(function (loggedInUser) {
        getCurrentTicket().then(function (ticket) {
            let data = {
                'agentId': loggedInUser.id,
                'ticketId': ticket.id,
                'ticketSubject': document.getElementById('notes').value + '. $$'+ ticket.subject
            };
